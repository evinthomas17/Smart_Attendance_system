...
const loadSubjectsAndFaculty = useCallback(async (classId) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.response) {
        console.error("Timetable update error:", err.response.data);
        if (err.response.status === 400) {
          ...
          if (errorData.academic_class) {
            ...
            setError(Array.isArray(msg) ? msg.join(", ") : (msg || "Failed to update timetable. Please check your input."));
          }
          ...
          setError(Array.isArray(msg) ? msg.join(", ") : (msg || "Failed to update timetable. Please try again."));
        }
      } else if (err.request) {
        setError("Network error. Please check your connection.");
      } else {
